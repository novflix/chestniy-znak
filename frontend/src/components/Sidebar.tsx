...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 shrink-0 h-screen sticky top-0 flex flex-col bg-gray-900 border-r border-gray-800">
      {/* Logo */}
      <div className="px-4 py-5 border-b border-gray-800">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-bold text-white leading-tight">Маркировка</p>
            <p className="text-[10px] text-green-400/70 font-medium leading-tight">«Честный знак»</p>
          </div>
        </div>
      </div>

      {/* Nav — ADMIN sees all, USER sees nothing extra */}
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {isAdmin ? (
          ...
          /* USER — only verify link */
          <Link href="/verify"
            className={`nav-item ${pathname === '/verify' ? 'active' : ''}`}>
      ...
      <div className="px-3 py-4 border-t border-gray-800">
        <div className="flex items-center gap-2 px-3 py-2 mb-2">
          <div className="w-7 h-7 rounded-full bg-green-500/20 border border-green-500/30 flex items-center justify-center text-xs font-bold text-green-400 shrink-0">
        ...
        <button onClick={logout}
          className="w-full nav-item text-gray-500 hover:text-red-400 hover:bg-red-900/10">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
